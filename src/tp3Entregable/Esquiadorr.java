...
class Esquiadorr extends Thread {
	private int id;
	private Semaphore sillasSemaphore;

	public Esquiadorr(int id, Semaphore sillasSemaphore) {
		this.id = id;
	...
	public void run() {
		try {
			while (true) {
				...
				System.out.println("El esquiador " + id
						+ " se sube a la aerosilla en la base.");
				Thread.sleep(1000); // Simulación de tiempo en el ascenso de la silla
				// Libera la silla en la cima
				System.out.println("El esquiador " + id
						+ " se baja de la aerosilla en la cima. "
						+ "Empieza a descender por la montaña.");
				...
				System.out.println("El esquiador " + id
						+ " descendió hasta la base. Está listo para volver a subir.");
			}
